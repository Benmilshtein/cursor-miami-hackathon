...
async function loadJudgeData() {
  try {
    const data = await fetchJSON("/api/staff/repo-analyzer/judges");
    const map = new Map();
...
async function fetchAISummary(repoId) {
  ...
  const text = await fetchText(`/api/staff/repo-analyzer/repo/${encodeURIComponent(repoId)}/ai`);
...
async function renderSummaryTable(rows) {
  ...
  sortedRows.forEach((row) => {
    ...
    const displayName = row.project_name || row.team_name || row.repo_id || extractRepoName(row.repo);
    const subtitle = [row.team_name, row.repo].filter(Boolean).join(" · ");
    const judgeInfo = getJudgeInfoForRow(row);
    const scoreLink = row.team_id
      ? `<a class="score-link" href="/staff/evaluate/${row.team_id}" onclick="event.stopPropagation()" style="font-size:11px;color:#2ee6c5;text-decoration:none;margin-top:4px;display:inline-block">Score team →</a>`
      : "";
    ...
    tr.innerHTML = `
      <td>
        <div class="repo-cell">
          ...
          <span class="repo-url">${escapeHtml(subtitle)}</span>
          ${scoreLink}
        </div>
      </td>
...
async function loadSummary() {
  const [summaryData] = await Promise.all([
    fetchJSON("/api/staff/repo-analyzer/summary"),
    loadJudgeData(),
  ]);
...
async function loadDetails(repoId) {
  ...
  try {
    const [metrics, aiText, commitsData] = await Promise.all([
      fetchJSON(`/api/staff/repo-analyzer/repo/${encodeURIComponent(repoId)}/metrics`),
      fetchText(`/api/staff/repo-analyzer/repo/${encodeURIComponent(repoId)}/ai`),
      fetchJSON(`/api/staff/repo-analyzer/repo/${encodeURIComponent(repoId)}/commits`).catch(() => ({ rows: [] })),
    ]);
...
async function runScan() {
  ...
  try {
    const res = await fetch("/api/staff/repo-analyzer/scan", {
      method: "POST",
      credentials: "include",
    });
    const data = await res.json();
    if (!res.ok || data?.success === false) {
      alert(data?.error?.message || data?.data?.stderr || "Scan failed");
    } else if (data?.data && data.data.ok === false) {
      alert("Scan finished with errors:\n" + (data.data.stderr || data.data.stdout || ""));
    }
    await loadSummary();
  } catch (e) {
    alert(String(e));
...
document.addEventListener("DOMContentLoaded", () => {
  const btn = document.getElementById("run-scan-btn");
  if (btn) btn.addEventListener("click", () => void runScan());
});
